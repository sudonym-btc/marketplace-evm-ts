...
async function settlementOperation(options: {
  ...
  if (existing) {
    if (existing.chainId !== options.chainId || existing.tradeId !== options.tradeId || !operationMatches(existing, options.data)) {
  ...
  if (options.store.putIfAbsent) {
    if (await options.store.putIfAbsent(record)) return record
    const raced = await options.store.get(options.id)
    if (!raced || raced.chainId !== options.chainId || raced.tradeId !== options.tradeId || !operationMatches(raced, options.data)) {
...
export async function replayCompletedEvmMarketplacePayment(options: {
  ...
  const operationId = `evm-order-${intent.action}-${intent.orderGroupId}-${intent.paymentId}`
  const record = await options.operationStore.get(operationId)
  if (!record || record.status !== 'completed') {
    ...
    const candidates = (await options.operationStore.list({ kind: 'escrow', status: 'completed', chainId }))
      .filter(candidate =>
        candidate.data.purpose === 'order'
        && candidate.tradeId?.toLowerCase() === tradeId.toLowerCase(),
      )
    if (candidates.length > 0) {
      throw new Error('EVM payment is already bound to a different completed settlement operation')
    }
    return null
  }
  ...
  if (completed.action !== intent.action) {
    throw new Error(`Completed EVM settlement ${operationId} is bound to ${completed.action}`)
  }
  return {
    type: 'completed',
    proof: intent.proof,
...
export async function* settleEvmMarketplacePayment(options: {
  ...
  const operation = await settlementOperation({
  ...
  yield {
    ...
    data: { operationId, action: intent.action },
  }

  try {
    let txHash = operation.txHash
    if (!txHash || operation.status !== 'completed') {
